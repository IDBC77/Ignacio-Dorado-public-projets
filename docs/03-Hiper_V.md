...
Finalmente, se procede a la preparación del disco desde la herramienta de administración de discos del sistema cliente. En primer lugar, el disco se activa manualmente (pasándolo a estado “En línea”), lo que permite posteriormente su inicialización, recomendándose el uso del estilo de partición GPT. Una vez inicializado, se crea un nuevo volumen simple ocupando el espacio disponible, asignándole una letra de unidad y formateándolo en NTFS. De este modo, el almacenamiento iSCSI queda completamente operativo y puede utilizarse como repositorio para máquinas virtuales, discos VHDX u otros recursos, reproduciendo un escenario real de almacenamiento centralizado en entornos profesionales.
